...
sleep timer); a native Kotlin/Jetpack Compose app + home-screen widget is the client.
...
- `app/android/` — native Kotlin/Jetpack Compose Android app, media service, and home-screen widget.
...
- **Testing**: pytest for the server (Hammerspoon shimmed via a fake `hs` binary);
  JVM tests and Android lint for the app. CI gates every release: red tests = no APK.
- **Git**: merge, never rebase. Small, frequent commits — the repo is the
  checkpoint store for agent resumability.
- **Releases**: tag `vX.Y.Z` → GitHub Actions: tests → native Gradle
...
- App tests/lint/build: `cd app/android && ./gradlew testDebugUnitTest lintDebug assembleRelease`

## Resume protocol (for any agent picking this up)

1. Read the newest relevant document in `docs/plans/`; the Expo implementation
   plan is historical, and the native migration design records the current client.
...
- Local unsigned APK builds are allowed for verification. Signed release APKs come from CI.
